...
exports.handler = async (event) => {
  ...
  const generateRecommendationLogic = (answers, recommendation, score) => {
    ...
    if (answers.length >= 7) {
      const teamSize = answers[1]?.value || 0;
      const timeline = answers[2]?.value || 0;
      const budget = answers[3]?.value || 0;
      const businessStage = answers[4]?.value || 0;
      const engagementType = answers[5]?.value || 0;
      const marketComplexity = answers[6]?.value || 0;
      
      if (teamSize <= 1) factors.push("Limited internal marketing resources");
      if (timeline <= 2) factors.push("Urgent timeline requirements");
      if (budget >= 3) factors.push("Substantial marketing budget available");
