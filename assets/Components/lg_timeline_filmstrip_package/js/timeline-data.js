// Timeline data.uses same structure as your current implementation.
// Image paths point to /assets/images/*. Update paths only if your directory changes.
// 'brands' can be replaced with real logo PNG/SVGs; 'brandsSupported.image.src' can be a composited image.

window.timelineData = {
  foundations: {
    title: "Foundations (2012–2014)",
    quote: "This era was my entry into the tech world - and into Microsoft's boldest bets.",
    description: "From the debut of <strong>Windows 8</strong> and the first <strong>Surface</strong> devices to supporting the <strong>Office 2013</strong> rollout, I guided in-store strategy and retail attach for Microsoft's hardware and software milestones.",
    achievements: [
      "<strong>Windows 8 & Surface</strong> launch support across retail channels",
      "Retail attach strategy for <strong>Office 2013</strong>",
      "Campaign optimizations for <strong>PC Accessories</strong>",
      "Product storytelling & marketing enablement online"
    ],
    brands: [
      { src: "assets/images/brand-windows8.png", alt: "Windows 8" },
      { src: "assets/images/brand-surface.png", alt: "Surface" },
      { src: "assets/images/brand-office2013.png", alt: "Office 2013" }
    ],
    brandsSupported: {
      title: "Brands I Supported",
      image: { src: "assets/images/brands-supported-foundations.png", alt: "Brands Supported.Foundations Era" }
    }
  },
  ascent: {
    title: "Ascent (2014–2016)",
    quote: "From retail-ready demos to full campaign orchestration.",
    description: "I expanded Microsoft's in-store experiences - launching categories like <strong>Surface Pro 3</strong>, <strong>HoloLens</strong>, and <strong>Microsoft Band</strong> - then moved into Integrated Marketing for <strong>Office</strong>, shaping cross-platform storytelling.",
    achievements: [
      "Office for iPad launch & cross-platform narrative",
      "Surface Pro 3 demo kit & campaign alignment",
      "Microsoft Band & HoloLens retail support",
      "Windows 10 attach strategy & Office GTM coordination"
    ],
    brands: [
      { src: "assets/images/brand-surface.png", alt: "Surface" },
      { src: "assets/images/brand-surface-pro3.png", alt: "Surface Pro 3" },
      { src: "assets/images/brand-hololens.png", alt: "HoloLens" }
    ],
    brandsSupported: {
      title: "Brands I Supported",
      image: { src: "assets/images/brands-supported-ascent.png", alt: "Brands Supported.Ascent Era" }
    }
  },
  rewrite: {
    title: "Rewrite (2016–2018)",
    quote: "Stepping fully into strategic storytelling for Office.",
    description: "Joining the Office Business Group, I led seasonal campaigns and partner go-to-market strategies, guiding the narrative arc from <strong>Office 2016</strong> to <strong>Office 2019</strong>.",
    achievements: [
      "Retail & partner marketing for Office 2016 lifecycle",
      "Early narrative direction for Office 2019",
      "Creative assets for seasonal & evergreen campaigns",
      "Translating product updates into consumer stories"
    ],
    brands: [
      { src: "assets/images/brand-office2016.png", alt: "Office 2016" },
      { src: "assets/images/brand-office2019.png", alt: "Office 2019" }
    ],
    brandsSupported: {
      title: "Brands I Supported",
      image: { src: "assets/images/brands-supported-rewrite.png", alt: "Brands Supported.Rewrite Era" }
    }
  },
  pivot: {
    title: "Pivot (2019–2020)",
    quote: "Reintroducing Microsoft 365 with a human lens.",
    description: "I led the consumer launch of <strong>Family Safety</strong> during the pandemic and steered the rebrand from Office 365 to <strong>Microsoft 365</strong>, building modular kits of parts for global scale.",
    achievements: [
      "<strong>Family Safety</strong> launch & animated story campaign",
      "Microsoft 365 rebrand partner rollout",
      "Scalable creative kits for retail & digital"
    ],
    brands: [
      { src: "assets/images/brand-microsoft365.png", alt: "Microsoft 365" },
      { src: "assets/images/brand-family-safety.png", alt: "Microsoft Family Safety" }
    ],
    brandsSupported: {
      title: "Brands I Supported",
      image: { src: "assets/images/brands-supported-pivot.png", alt: "Brands Supported.Pivot Era" }
    }
  },
  rise: {
    title: "Rise (2020–2023)",
    quote: "When the world shut down, storytelling became survival.",
    description: "I created the '<strong>Free to Be Free</strong>' campaign celebrating digital liberation, and previewed <strong>Teams for Life</strong> - blending lifestyle marketing with product narratives.",
    achievements: [
      "<strong>Free to Be Free</strong> brand campaign",
      "<strong>Teams for Life</strong> preview launch"
    ],
    brands: [
      { src: "assets/images/brand-teams.png", alt: "Teams" },
      { src: "assets/images/brand-free-to-be-free.png", alt: "Free to Be Free" }
    ],
    brandsSupported: {
      title: "Brands I Supported",
      image: { src: "assets/images/brands-supported-rise.png", alt: "Brands Supported.Rise Era" }
    }
  },
  reinvention: {
    title: "Reinvention (2023–2025)",
    quote: "Even AI needs a heartbeat.",
    description: "I led Microsoft Edge's shift into the AI-powered browser and shaped Copilot narratives - making smart shopping and productivity tools approachable through integrated, human-centered campaigns.",
    achievements: [
      "<strong>Edge-ucational Series</strong>",
      "<strong>Edge AI Campaign</strong>",
      "<strong>Mobile App Store Rebrand</strong>",
      "<strong>Personal Insights & Learnings</strong>"
    ],
    brands: [
      { src: "assets/images/brand-edge.png", alt: "Microsoft Edge" },
      { src: "assets/images/brand-copilot.png", alt: "Copilot" }
    ],
    brandsSupported: {
      title: "Brands I Supported",
      image: { src: "assets/images/brands-supported-reinvention.png", alt: "Brands Supported.Reinvention Era" }
    }
  }
};
