    ...
    const startAssessment = async (type: string) => {
        ...
        try {
            const res = await fetch('/api/assessment/session', {
                ...
                body: JSON.stringify({ client_id: selectedClientId, assessment_type: type })
            });
      ...
      return (
        <div className="space-y-4">
          ...
          {q.options.map((opt, i) => (
            <button 
              key={i} 
              className={`w-full text-left p-4 rounded-xl border border-[var(--border-strong)] bg-[var(--bg-alt)] hover:border-[var(--coral)] transition-all duration-200 ${assessmentState.answers[q.id]?.value === opt.value ? 'border-2 border-[var(--coral)]' : ''}`} 
                          ...
                          <div className="p-8 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-strong)] shadow-[var(--shadow-soft)]">
                              <div className="mb-8">
                                  <div className="flex items-center justify-between text-sm mb-2">
                                      ...
                                      <span className="font-bold text-[var(--coral)]">{(assessmentState.index / 2) * 100}%</span>
                                  </div>
                                  <div className="w-full bg-[var(--bg-alt)] rounded-full h-3">
                                      <div className="bg-[var(--coral)] h-3 rounded-full" style={{ width: `${(assessmentState.index / 2) * 100}%` }}></div>
                                  </div>
                              </div>
                            ...
                            <div className="p-6 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-strong)] shadow-[var(--shadow-soft)]">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between text-[var(--text-secondary)]">
                                        ...
                                        <span className="font-bold text-[var(--teal)]">${recommendedTier.price.replace('$', '').split('-')[0].trim()}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-[var(--text-secondary)]">
