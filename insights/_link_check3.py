...
ROOT = r"C:\\Users\\luisg\\OneDrive\\Documents\\Luis Gilberto"
...
def is_external(url):
    ...
    if not p.scheme:
        return False
    return p.netloc not in ("localhost", "[::]", "127.0.0.1")

resp = requests.get(urljoin(BASE, PAGE), timeout=15)
...
for text, href, tag in links_live:
    if not href:
        continue
    test_url = href if urlparse(href).scheme else urljoin(BASE, href)
    ...
    try:
        ...
        final_url = head.url
        if external and urlparse(final_url).scheme != "https":
